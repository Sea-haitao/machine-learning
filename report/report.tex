...
We compare three clustering algorithms: K-means (centroid-based, using Euclidean distance),
Agglomerative Hierarchical Clustering (Ward linkage, bottom-up
merging), and Gaussian Mixture Models (probabilistic, full
covariance). For each method, we search $K \in \{2, \ldots, 10\}$ and evaluate 
cluster quality using three internal metrics (Silhouette, Calinski--Harabasz,
...
\textbf{Determination of the Clustering Method:} K-means with $K{=}3$ is selected 
as the primary clustering method and GMM as a supplementary reference. Across all 
considered internal evaluation metrics, K-means achieves the clearest internal separation. 
GMM is employed as a complementary reference for probabilistic assignment, as its soft 
assignment mechanism facilitates the interpretation of ambiguous sample points. Though 
agglomerative clustering shows some competitive structures at certain values of $K$, 
it does not consistently outperform K-means. 
\newline Moreover, the corresponding external scores are modest, indicating that the unsupervised 
clusters only partially correspond to the academic outcome labels. Such moderate agreement 
with ground-truth labels is typical in educational datasets, where student outcomes are shaped 
by multi-dimensional factors that cannot be fully captured by distance-based partitioning alone. 
This observation aligns with the t-SNE visualization where the Enrolled class is not cleanly 
separable in low-dimensional space.
